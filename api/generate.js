...
module.exports = async (req, res) => {
  ...
  req.on('end', async () => {
    try {
      ...
      const allItems = [];
      const sources = [['weibo',2],['zhihu',2],['thepaper',1],['tencent',1]];
      ...
      // 字符相似度去重
      const deduped = deduplicate(allItems, 8);
      ...
      // 超过6条时用AI二次去重
      const finalItems = validItems.length > 6 ? await aiDeduplicate(validItems) : validItems;
...
async function aiDeduplicate(items) {
  ...
  const prompt = `以下是来自不同平台的热点标题，请将属于同一事件的不同角度报道合并。输出一行一个，用 | 分隔合并后的标题和原始标题，格式：合并后标题 | 原标题1 | 原标题2\n\n${titles}`;
      ...
      pr.on('end', () => {
        try {
          ...
          for (const line of lines) {
            ...
            if (parts.length >= 2) {
              ...
              for (let i = 1; i < parts.length; i++) {
                const idx = items.findIndex(it => it.title === parts[i]);
          ...
          for (let i = 0; i < items.length; i++) {
            if (mergeMap[i] !== undefined) mergedInto[i] = mergeMap[i];
            else if (!usedIndices.has(i)) result.push(items[i]);
          }
