...
async function processGenerate(targetDate, targetLabel, jobId, redisUrl, redisToken) {
  try {
    ...
    // 获取热点 - 增加数量以弥补过滤损失
    const allItems = [];
    for (const [sourceId, limit] of [['weibo',5],['zhihu',5],['thepaper',3]]) {
    ...
    // 先去重（宽松条件），保留足够候选
    const candidates = deduplicate(allItems, 10);
    ...
    for (const item of candidates) {
      ...
      // 过滤：AI返回null表示娱乐/体育内容，跳过；defaultAI也跳过
      const isDefaultAI = item.ai && item.ai['知识点'] && item.ai['知识点'][0] === '待补充';
      if (item.ai !== null && !isDefaultAI) {
        validItems.push(item);
      }
      if (validItems.length >= 3) break; // 已收集足够数量就停
    }
    ...
    // 保存到Redis
    const payload = { date: targetLabel, items: validItems, generatedAt: new Date().toISOString() };
    ...
    await saveStatus(jobId, { status: 'done', count: validItems.length, date: targetDate });
    ...
    const req = https.request({
      ...
      pr.on('end', () => {
        try {
          ...
          resolve(result); // parseAI returns null if should be skipped
...
function parseAI(content) {
  ...
  try {
    ...
    // 过滤标准：null对象或明确返回null
    if (parsed === null || (typeof parsed === 'object' && parsed !== null && parsed.length === 0)) {
      return null;
    }
    if (parsed && Array.isArray(parsed.知识点) && Array.isArray(parsed.怎么考)) {
      // 检查是否所有知识点都是"待补充"，如果是则视为无效内容
